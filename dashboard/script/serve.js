...
} from '../../vendor/github.com/sensu/sensu-go/dashboard/client.webpack.config';
import libConfig, {outputPath as libOutputPath} from '../../vendor/github.com/sensu/sensu-go/dashboard/lib.webpack.config';
import moduleConfig, {outputPath as moduleOutputPath} from '../webpack.config';

const clientStats = require(path.join(clientOutputPath, 'stats.json'));
const libStats = require(path.join(libOutputPath, 'stats.json'));
const moduleStats = require(path.join(moduleOutputPath, 'stats.json'));

const app = express();

app.use(express.static(libConfig.output.path));
app.use(express.static(clientConfig.output.path));
app.use(express.static(moduleConfig.output.path));
